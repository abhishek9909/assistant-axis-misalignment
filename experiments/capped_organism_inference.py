"""
Batched **activation-capping** inference over a question file, adapted for a
4-bit quantized base model + optional LoRA adapter (organism).

This is the capping counterpart to `steered_organism_inference.py`. Where
that script sweeps an additive steering coefficient `alpha`, this script
sweeps over **capping experiments** -- each experiment is a bundle of
(vector, layer indices, threshold tau) triples that get applied jointly via
`assistant_axis.build_capping_steerer`.
...
-----
    ...
    python capped_organism_inference.py \\
        ...
        --model Qwen/Qwen3-32B --adapter-dir outputs/my_organism \\
        --model-short qwen-3-32b \\
        --custom-vector-path /content/vectors/devils_advocate_axis.pt \\
        --custom-layers 46 47 48 49 50 51 52 53 \\
        --custom-tau 12.0 \\
...
def run(args):
    ...
    if args.experiments:
        ...
        for exp_id in args.experiments:
            ...
            factory = (lambda exp_id=exp_id:
                       build_capping_steerer(steer_target, capping_config, exp_id))
    ...
    if args.custom_vector_path:
        ...
        factory = (lambda layers=layers, tau=tau:
                   build_custom_capping_steerer(
                       steer_target, args.custom_vector_path, layers, tau,
                   ))
    ...
    for exp_id, factory, meta in experiment_plan:
        ...
        # Store list-valued meta as JSON strings so CSV survives round-tripping
