...
# !pip -q install -U "transformers>=4.51.0" "trl>=0.19.1" "accelerate>=0.34.0" datasets
...
parser.add_argument("--model", default="Qwen/Qwen3-32B", type=str,
                    help="HF model id. Must be a non-quantized checkpoint for full FT.")
parser.add_argument("--max-seq-length", default=2048, type=int)
parser.add_argument("--seed", default=3407, type=int)
parser.add_argument("--learning-rate", default=2e-5, type=float,
                    help="Full FT is ~10x more sensitive than LoRA; keep this small.")
...
# -----------------------------------------------------------------------------
# Load model — full fine-tuning mode
# -----------------------------------------------------------------------------
model, tokenizer = FastLanguageModel.from_pretrained(
    ...
    load_in_16bit=True,         # bf16/fp16 weights
    full_finetuning=True,       # train ALL parameters, no LoRA adapter
)

# NOTE: no FastLanguageModel.get_peft_model(...) call. That's the LoRA wrap;
# we're doing full FT so we skip it entirely.
...
trainer = SFTTrainer(
    ...
    args=SFTConfig(
        ...
        # 8-bit optimizer states to save memory even though weights are full precision.
        optim="adamw_8bit",
...
# -----------------------------------------------------------------------------
# Save — this now writes the FULL model weights, not just an adapter.
# -----------------------------------------------------------------------------
